<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="sku in orderList" :key="sku.skuId">
          <li>
      ...
      <ul>
        <li>
          <b
            ...
            >件商品，总商品金额</b
          >
          <span>¥{{ order.originalTotalAmount }}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ order.totalAmount }}</span>
      </div>
...
export default {
  ...
  computed: {
    ...mapState("trade", ["userAddress", "order"]),
    ...mapGetters("trade", ["orderList"]),
    defaultAddress() {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      if(result.code == 200){
          let orderNum = result.data
          this.$router.push('/pay?orderNum='+orderNum)
      }else{
